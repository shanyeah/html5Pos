...
import './PosGoodsView.css';
import MoneyValue from '../../utils/money';

class PosGoodsView extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            currentCategory: 0,
                        ...
                        <List.Item >
                            <div className='pos_goods_item' data-item={JSON.stringify(item)} onClick={event => {
                                ...
                                if (item.type == 14) {
                                    ...
                                    for (let i = 0; i < item.goodsPackList.length; i++) {
                                        ...
                                        for (let j = 0; j < element.items.length; j++) {
                                            ...
                                            if (itemElement.goodsTagCategories && itemElement.goodsTagCategories.length > 0) {
                                                ...
                                                for (let k = 0; k < itemElement.goodsTagCategories.length; k++) {
                                                    ...
                                                    if (tagElement.goodsTags.length > 0) {
                                                        ...
                                                        itemDict.selectTag[tagElement.id] = firstTag.id; // 记录选中的口味
                                                    }
                                                }
                                            ...
                                            for (let index = 0; index < itemDict.quantity; index++) {
                                                itemElement.uniqueId = itemDict.goodsId + "" + index;
                                                itemDict.uniqueId = itemElement.uniqueId;
                                                items[itemDict.uniqueId] = itemDict;
                                                if (Object.keys(selectItems).length < element.quantity) {
                                                    selectItems[itemDict.uniqueId] = itemDict;
                                                }
                                            }
                                        }
                                    ...
                                    console.log(selectGoodsPackList);
...
export default connect(mapStateToProps)(windowSize(PosGoodsView));